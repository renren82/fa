...
path_root = "H:/"
code_str = 'M0'
file_path = path_root + code_str + '.xlsx'
file_out_path = path_root + code_str + '_new.xlsx'
freq = 15
path_param = 'future_config.xlsx'
...
def get_data(codename, freq):
    ...
    for line in res_json:
        # print(line)
        column = {}
        date_format = datetime.datetime.strptime(line[0], '%Y-%m-%d %H:%M:%S')  # 格式化日期
        column['trade_date'] = date_format.strftime("%Y%m%d%H%M%S")
        column['open'] = float(line[1])
        column['high'] = float(line[2])
        column['low'] = float(line[3])
        column['close'] = float(line[4])
        column['vol'] = int(line[5])
...
if __name__ == '__main__':
    ...
    print(df_new.head())
    #---------------- -up is data ------------------------
    base_delta, base_price, base_power = delta_base_3ma_5ma(df_new, 'waveup', '20201023095000', '20201023213500')
